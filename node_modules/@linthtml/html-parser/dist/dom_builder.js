...
class Handler extends htmlparser2_1.DomHandler {
    constructor(lineOffsets) {
        ...
        this.root = new dom_utils_1.Document(this.dom);
        this.attributes = [];
    ...
    __createAttributeNode(name, attribute_value) {
        ...
        if (rawEqValue) {
            const loc = new dom_utils_1.Range(namePosition.end, this._indexToPosition(start + name.length + rawEqValue.length));
            equal = new dom_utils_1.CharValue(rawEqValue, loc);
        }
        if (attribute_value) {
            const rawValue = raw.slice(rawEqValue.length);
            const loc = new dom_utils_1.Range(this._indexToPosition(start + name.length + rawEqValue.length), this._indexToPosition(end));
            // remove extra spaces newline? (attribute_value)
            value = new dom_utils_1.CharValue(attribute_value, loc, rawValue);
        }
        else {
            end = start + name.length;
        }
        return new dom_utils_1.NodeAttribute(new dom_utils_1.CharValue(name, namePosition), {
            start: this._indexToPosition(start),
    ...
    onopentag(name /* , attribs: { [key: string]: string } */) {
        ...
        const node = new dom_utils_1.Element(name, this.attributes, undefined, type);
        this.addNode(node);
