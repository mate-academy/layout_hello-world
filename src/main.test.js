...
const getSiteBody = (linkText) => {
  const fileContent = fs.readFileSync('readme.md', 'utf8');
  const escapedLinkText = linkText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const regex = new RegExp(`\\[${escapedLinkText}\\]\\((.*?)\\)`);
  const match = fileContent.match(regex);
  if (!match) {
    throw new Error(`Could not find URL for ${linkText}`);
  }
  const url = match[1];
...
describe('Environmental Check', () => {
  ...
  test('You should have Git of 2.31.1 version or newer', () => {
    ...
    if (OS === 'Windows') {
      expect(Number(version) >= minVersionOfGitOnWindows).toBeTruthy();
    } else {
      expect(Number(version) >= minVersionOfGitOnMacAndLinux).toBeTruthy();
    }
  });
